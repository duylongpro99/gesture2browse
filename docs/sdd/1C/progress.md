...
| 9 | docs | `c6841eb` | **done** | arch §2/§3.1/§3.2 direct-pointer route → SW-relay `PageCommand{pointer}` (ADR 0001), direct offscreen→CS port kept reserved; +1 bullet in background.md/protocol.md; §4.1 sequence-diagram pointer label canonicalized. Boundary-lint OK; exit-check unchanged (docs-only). |

## Session 5 (execute, Task 9; via `obra-subagent-driven-development`)

Done: Task 9 (`[docs]`, the one task owned per `.claude/scope.json`) — the owner-directed (answer #4, 2026-09-13) arch-doc + rule update recording accepted **ADR 0001** (pointer plane via SW relay), per CLAUDE.md §3 step 4 / §5 (same PR as the deviation). One implementer subagent (sonnet) + task review (spec + quality + boundary gate, sonnet) + 1 fix round + scoped re-review (haiku).

- **Task 9** (`a9c46f0` + fix `c6841eb`): `docs/02-architecture.md` §2 diagram (removed the direct `WK→CS PointerUpdate` edge; pointer now rides `WK→SW` in `GestureFrame` and `SW→CS` as `PageCommand{pointer} @frame rate (ADR 0001)`), §3.1 offscreen pseudocode (`// ADR 0001`: no direct-to-CS pointer port; SW relays), §3.2 dispatcher prose (pointer routed via SW as `PageCommand{type:'pointer',x,y,state:CursorState}` over the frozen `ServiceWorkerToContent` port, one coalesced msg/frame, ADR 0001; direct offscreen→CS `MessagePort` kept **reserved** with the MV3 rationale), and §4.1 sequence diagram (`PointerUpdate{x,y}` → `PageCommand{pointer x,y}`). `.claude/rules/background.md` += SW-relays-pointer bullet; `.claude/rules/protocol.md` += `pointer`-variant bullet. Arch §6/§8 untouched (already matched the accepted route). No code/schema/test change.

Rulings (CLAUDE.md §1 / SDD, in `sdd-5-ledger.md`): (1) §3.1 edited alongside the brief's §3.2/§2 for coherence (same direct-route assertion). (2) rule-file bullets added per impl plan Step 2 + scope.json even though ADR line 31 says none strictly required — additive, no boundary loosened. (3) the §4.1 pointer-name Minor was fixed (not deferred) because it names the exact message ADR 0001 canonicalizes; adjacent `HoverTarget`/`Feedback` labels left untouched (out of scope).

Verification: `node scripts/lint/boundary-lint.mjs` OK; `git diff --stat` scope-correct each commit; `scripts/milestone/exit-check 1C --fast` unchanged by a docs-only task (all 8 rows PASS at c6841eb — see handoff).
