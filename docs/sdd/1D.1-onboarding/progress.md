...
|---|---|---|---|---|
| 1 | protocol | — | todo | New `Profile` (default `accessibility`), `Settings`, `OnboardingState`, `OnboardingComplete` (additive, zod-only). Turns I1/I2 green. |
| 2 | onboarding (new) | — | todo | Pure DOM-free `steps.ts`: device-presence, camera-step, host-access mode, next-step, default profile. |
