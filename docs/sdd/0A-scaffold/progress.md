...
| 5 | playground — bench harness with CSV export, headless bench | `.claude/rules/fixtures-and-tests.md` | done | 5e5dee3 |
| 6 | extension — buildable WXT (MV3, React) skeleton | — | done | a8682ab |
| 7 | CI + trust-boundary lint rules | `.claude/rules/fixtures-and-tests.md` | done | 4ccf34f |
| 8 | docs/spike-results.md template (0.12) | — | done | 47bd886 |
...
| E3 | bench harness runs headless on a placeholder y4m | PASS (Task 5: apps/playground bench e2e) |
...
- 2026-09-05 (session 5, execute): Tasks 7 and 8 done — **all 8 execute tasks complete, milestone DONE.** Task 7 (4ccf34f) — `.github/workflows/ci.yml` (Node 24, pnpm: install/build/lint/typecheck/test, Playwright Chromium + E3 bench e2e, extension zip, artifact upload) and `scripts/lint/boundary-lint.mjs`, a dependency-free grep lint (comment-stripped) enforcing CLAUDE.md §2: VideoFrame/ImageBitmap only in offscreen, apiKey/API_KEY only in background.ts, no confirm(), gesture-timing constants only in gesture-core, content ↛ agent-package import. Wired into `pnpm lint` + a `lint:boundary` script; regexes and clean-repo exit-0 verified (negative test done via `node -e`, no committed failing fixture per plan Step 2 — scope hook blocked planting temp files). Folded in the session-3 root workspace:* devDeps (`@gesture/protocol`, `@gesture/gesture-core`) so `scripts/fixtures/*.ts` resolve from repo root. Task 8 (47bd886) — `docs/spike-results.md` empty template: one section per gate G1–G8 with threshold + producing milestone/owner + Setup/Result(numbers)/Gate-met, go/no-go table, committed-numbers checklist, owner sign-off pointing G8 to roadmap §8. Exit check (`--fast`): **all 6 PASS** (E1/E2/E3/I1/I2/I3), lock OK. Full clean-clone E1 (without `--fast`) is the finish session's job.
...
- 2026-09-05 (session 4, execute): Tasks 5 and 6 done. Task 5 (5e5dee3) — `apps/playground` Vite bench app: `runBench` times capture/infer/normalize/classify/filter over the fake camera through MediaPipe `HandLandmarker` and emits one `BenchRow` (validated) serialized by `benchToCsv` (header == `BENCH_COLUMNS`); headless e2e (E3) drives it on `fixtures/bench/placeholder.y4m` and asserts a non-empty CSV. `hand_landmarker.task` (float16, 7.5 MB) committed at repo root; wasm + model served locally from `vite.config.ts` (no CDN). Owner-run recorder page (`recorder.html` + `recorder.ts`) included. Task 6 (a8682ab) — WXT 0.21 (MV3) + React 19 extension skeleton: background/content/offscreen(+inference.worker stub)/sidepanel(React)/grant-camera entrypoints, `wxt build`+`wxt zip` green. Exit check (`--fast`): **all 6 PASS** (E1/E2/E3/I1/I2/I3). Deviations recorded in plan `## Status`. Next: Tasks 7 (CI + boundary lint) and 8 (spike-results template).
