...
| 2026-09-06 | G7 (0E) agent latency probe = GO. Provider 9router (OpenAI-compatible gateway); fast + planner model glm-5.2 (first-suggestion p50 2653 ms / p95 3886 ms; tool-calling Y, json_schema Y). Owner chose glm-5.2 for fast over also-measured deepseel-v4-flash (p50 1574 ms, json_schema-only) to enable tool-calling; deepseel-v4-flash kept as a lower-latency tool-free alt. p50 within 3 s met; unblocks 2A. | G7 (0E): E1 owner live run 2026-09-06 (9router), E3 stub Vitest | Recorded in spike-results.md §G7 |
| 2026-09-06 | **G1 (0B) frame pump = GO on the browser path.** Offscreen `MediaStreamTrackProcessor` → Worker → MediaPipe pump clears ≥ 28 fps with no rAF/timer dependence and no visible surface driving it: E2 (agent) 30.0 fps p05 over 60 s (WebGL, `numHands:1`); E1 (owner M1) 30.5 fps sustained over 10 min. Platform note: offscreen docs report `document.hidden===false` — "hidden" here means never-rendered / no visible surface, which the offscreen doc satisfies structurally (owner accepted reading A). Feeds the G8 browser-inference vs ONNX-Web decision. | G1 (0B): E2 agent 60 s p05 + E1 owner M1 10 min | Recorded in `spike-results.md §G1` |
| 2026-09-06 | **G8 (provisional) = GO on browser inference, to unblock 1A only.** 1A is the vertical slice: it fixes the shared `GestureFrame` / `Intent` / `PageCommand` / `PageEvent` / FSM interfaces through one gesture end to end and does not exercise the weak-machine inference path. Browser frame-pump is already GO (G1: 30 fps on M1 + agent). The **full** browser-inference vs ONNX-Web decision — which requires the 2020 Intel Air G3 bench — is **deferred to 1B**, where §9's fallback trigger already lives (Intel Air < 20 fps on every path → ONNX-Web path into 1B). G3 bench runs in parallel and does not pre-empt this provisional GO. | G8 (provisional; G1 met, G3 bench deferred to 1B) | Unblocks 1A only; the browser-vs-ONNX-Web call is re-made in 1B on real 2020 Intel Air numbers. Supersede this row with the final G8 = GO/NO-GO once `spike-results.md §G8` is signed. |
...
| 2026-09-12 | **[MOCK — fabricated to unblock 1C; REAL G6 STILL PENDING] G6 (Fitts / ergonomics) = GO (mock).** Placeholder defaults to test-drive the 1C plan, exactly as the G4 MOCK row did for 1B: snapping **ON** by default, **initial snap radius 40 px**, general click-mode default **pinch**, **dwell (600 ms)** as the Accessibility-profile default. No participants run; numbers fabricated (`spike-results.md §G6`), carries no real gate decision. To realify: run the real Fitts study (owner + 2), replace this row and the §G6 block with real data, and enter the numbers in `03-tech-stack §4`. | **MOCK (not a real G6 result)**; G6 | Recorded as MOCK in `spike-results.md §G6`; unblocks 1C (test-drive only) |
| — | Browser inference vs ONNX-Web on Intel Air | G3, G8 | pending; unblocks 1B |
| 2026-09-07 | **[MOCK — 1B WAS BUILT AGAINST THIS; REAL G4 + E1 STILL PENDING] G4 (launch gesture set) = GO (mock).** Placeholder six-gesture set used to test-drive the 1B pipeline: `palm-arm`, `fist-scroll`, `pinch-select`, `point`, `open-hand-pause`, `none`. Precision/recall targets and owner fixtures NOT collected — fabricated on the owner's explicit request to exercise the driver flow; carries no real gate decision. 1B is merged against it (pipeline, classifier seam, FSM, offscreen worker all real and reviewed), but Exit E1 (precision/recall) is unverified. | **MOCK (not a real G4 result)** | Not recorded in `spike-results.md`. To realify 1B: collect real G4 fixtures → re-run `scripts/train/train-gesture-mlp.ts` → confirm E1 → replace this row with the real G4 decision and remove the `pending` row below. |
