...
| 2026-09-07 | **[MOCK — TEST DATA, REVERT BEFORE REAL 1B] G4 (launch gesture set) = GO.** Placeholder six-gesture set to exercise the 1B driver flow: `palm-arm`, `fist-scroll`, `pinch-select`, `point`, `open-hand-pause`, `none`. Precision/recall targets and owner fixtures NOT collected — this row is fabricated by the driver on the owner's explicit request to test-drive 1B and carries no real gate decision. | **MOCK (not a real G4 result)** | Not recorded in `spike-results.md`; delete this row and restore the `pending` row below before running 1B for real |
