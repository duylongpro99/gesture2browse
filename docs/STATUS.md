...
|---|---|---|---|---|
| **1D.1** | drv-1D.1 | **Execute (s4): NEEDS-OWNER (E1 review).** All five tasks done. Task 5 @9b765f7: Playwright `onboarding` project + `onboarding.e2e.ts` (y4m fake webcam) — keyboard-only happy path (focus-per-step, aria-live, no `<video>`/`<canvas>`), `Settings` round-trips `storage.sync` re-parsed by `SettingsSchema` (**E2 green**), `OnboardingState` in `storage.local`, `test-results/onboarding.png` captured for **E1**; unhappy paths (no webcam, allow-this-time/denied re-route, restricted `<all_urls>`) covered. `tsc`/lint/boundary clean; `exit-check --fast` = **3 PASS (E2,I1,I2) 1 OWNER (E1)**, lock OK. Awaiting owner: run E1 `### Owner steps` in the journal + confirm the screenshot → milestone exits, owner logs §8. | docs/plans/1D.1-onboarding.md | 2026-09-14 |
