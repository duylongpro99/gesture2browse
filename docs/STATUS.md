...
| 0C G2 camera grant probe | drv-0C | **Plan written (session 0, docs-only).** `docs/plans/0C-camera-grant.md`: grant page + background `permissions.query` pre-check gate + cross-session "Allow this time" detection; new `protocol` `CameraGrantStatus`; new rule `.claude/rules/grant-camera.md`. Handoff CONTINUE — next session's scope derived from the **Files:** block. | `docs/plans/0C-camera-grant.md` | 2026-09-05 |
