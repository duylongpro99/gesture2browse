...
| 0B G1 frame pump probe | drv-0B | **Files block done + gate green (session 2).** `PumpStat` in protocol; offscreen `MediaStreamTrackProcessor`→Worker→MediaPipe pump + `fps-logger`; `background.ts` relay; **E2 passes: 30.0 fps p05 (WebGL), 60 s, no rAF**; lock refrozen at `14f62d9`. Owner accepted reading A (offscreen `document.hidden`=false is fine; E2 unchanged). Proposed §8: **G1 = GO** on the browser frame-pump path. **Only E1 pending** — owner's 10-min M1 run, numbers into `spike-results.md §G1`; DONE follows. | `docs/plans/0B-frame-pump.md` | 2026-09-05 |
