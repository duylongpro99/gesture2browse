...
|---|---|---|---|---|
| **1D.1** | drv-1D.1 | **Execute (s3): CONTINUE.** Tasks 3–4 done: onboarding page + `.claude/rules/onboarding.md` @2bcf4cf (full-tab wizard, no video, hostile-blob Zod validation); background first-run trigger/gate/profile wiring @3692b56 (onInstalled→onboarding, gate skips pump until complete, `Settings.profile`→dispatcher, dispatcher `Profile`→protocol enum, +8 gate unit tests). `tsc`/lint/boundary clean, 135 unit tests; `exit-check --fast` 2 PASS (I1,I2) 1 FAIL (E2 = Task 5 e2e, not written) 1 OWNER (E1). Next: Task 5 (Playwright e2e → E2 green + unhappy/a11y paths; E1 owner screenshot). | docs/plans/1D.1-onboarding.md | 2026-09-14 |
