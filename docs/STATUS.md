...
|---|---|---|---|---|
| 1B | drv-1B | **Impl tasks 1–7 + fix tasks 8–9 done; E2 PASS + E3 PASS; E1 owner-deferred.** Session 6 fixed findings 1–4: Task 8 `[gesture-core]` `Paused` re-arm asymmetry (`entry: clearClutch`), Task 9 `[offscreen]` real-surface `webglcontextlost` recovery (own the GL canvas) + sustained-cost delegate choice + `featureVersion` validation; new unit tests + a recovery-survival e2e. **CONTINUE — Task 10 remains:** make `frame-pump.e2e.ts` (0B G1 gate) idle-aware (out of session-6 scope; needs its own `[extension]` session). After 10: finish/merge, then owner real-browser context-loss check. Findings 5 + live palmFacing z-sign stay owner/real-camera. | `docs/plans/1B.md` | 2026-09-07 |
