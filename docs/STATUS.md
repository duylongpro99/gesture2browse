...
|---|---|---|---|---|
| **1D.1** | drv-1D.1 | **Finish (s5): DONE — PR open.** All five tasks merged on-branch; §5 verify green (tsc 8/8, lint+boundary OK, 8/8 test tasks); full `exit-check 1D.1` = **3 PASS (E2,I1,I2) 1 OWNER (E1)**, lock OK, 0 FAIL. PR to `master` opened ([#13](https://github.com/duylongpro99/gesture2browse/pull/13)). Remaining is owner-only: run E1 `### Owner steps` in the journal + confirm `test-results/onboarding.png`, then merge the PR and log the drafted §8 row + remove this row. | docs/plans/1D.1-onboarding.md | 2026-09-15 |
