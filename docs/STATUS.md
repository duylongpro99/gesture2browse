...
| 0B G1 frame pump probe | drv-0B | **Files block implemented (session 2, execute).** `PumpStat` in protocol; offscreen `MediaStreamTrackProcessor`→Worker→MediaPipe pump + `fps-logger`; `background.ts` relay; E2 Playwright gate **passes: 30.0 fps p05 (WebGL), 60 s, no rAF**. `tsc`/eslint/boundary-lint/vitest green. **NEEDS-OWNER:** (1) offscreen `document.hidden` is `false` in Chrome — confirm E2/E1 prove the gate's substance not `hidden===true`; (2) run the owner 10-min M1 hidden-doc check (E1). Numbers in `docs/spike-results.md §G1`. | `docs/plans/0B-frame-pump.md` | 2026-09-05 |
