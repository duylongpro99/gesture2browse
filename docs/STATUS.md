...
| 0C G2 camera grant probe | drv-0C | **Implemented (session 1); E2 green, E1 pending owner.** Files block done with TDD: `protocol` `CameraGrantStatus`, pure `permission.ts` helper, full-tab grant page, background pre-check gate before `ensureOffscreen()`, new rule `.claude/rules/grant-camera.md`, `camera-grant.e2e.ts`. E2 passes (grant→persistent status→gated offscreen `getUserMedia`, no prompt); **spike finding: MV3 SW _can_ `permissions.query({name:'camera'})`**. G1 unregressed. Handoff **NEEDS-OWNER**: E1 Chrome-restart + "Allow this time" check (steps in handoff), then §8 go/no-go. | `docs/plans/0C-camera-grant.md` | 2026-09-05 |
