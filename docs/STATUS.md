...
|---|---|---|---|---|
| 1B | drv-1B | **All impl tasks 1–7 + owner-authorized fix tasks 8–10 done; E2 PASS + E3 PASS; E1 owner-deferred. Branch ready for owner merge.** Session 7 (Task 10, `[extension]`, `bace638`) made `frame-pump.e2e.ts` (0B G1 gate) idle-aware — measures p05 over the active seed window only, so Task 5's adaptive-fps idle downshift no longer regresses it; the absolute 28 fps stays the owner-hardware G1 claim (headless caps active ~21.5 fps by detect cost). **DONE — no ledger task remains.** Next: `obra-finishing-a-development-branch` (owner merge), then owner real-browser context-loss check (finding 2) + owner-hardware G1 re-confirm. Findings 5 + live palmFacing z-sign stay owner/real-camera. | `docs/plans/1B.md` | 2026-09-07 |
