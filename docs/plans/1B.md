...
**Next (execute):** Tasks 5–7 in order (worker robustness + wire the trained classifier → restart-on-stream-end → adaptive-fps e2e E3). Task 5 is next; it wires the offscreen `palmFacing()`/`MlpClassifier`/landmark filter into the emitted `GestureFrame` (carry: confirm the `palmFacing` z-sign against live MediaPipe z). Threshold/weights changes replay the golden suite. exit-check: E2 PASS; E3 FAIL until Task 7; E1 owner-deferred.
