...
**Next (execute):** Tasks 3–7 in order (offline MLP training + shipped weights → FSM vote/palm-facing gate + golden replay suite E2 → worker robustness + wiring → restart-on-stream-end → adaptive-fps e2e E3). Task 3 is next. Threshold/weights changes replay the fixture suite. exit-check E2/E3 stay FAIL until Tasks 4/7 land; E1 owner-deferred.
