...
**In progress:** none. All impl tasks (1–7) committed on branch `1B`.

**Next:** **NEEDS-OWNER** (`docs/sdd/1B/handoff.md`). The mechanical exit checks pass (E2, E3) and `pnpm test` is green, but the final review found confirmed correctness bugs outside session 5's scope — including a non-functional context-loss recovery (a Task-6 deliverable). `obra-finishing-a-development-branch` (merge, owner territory) should wait until the owner authorizes follow-up fix sessions for findings 1–2 (and decides on the 3 minors + the `frame-pump.e2e` regression) vs. defers them.
